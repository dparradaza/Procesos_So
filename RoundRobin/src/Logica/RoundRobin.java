...
public class RoundRobin extends Thread {
    ...
    public RoundRobin(Ventana ventana) {
    ...
    public void run() {//atender los procesos
        ...
        listos.imprimir();
        listos.imprimirListos(ventana.getTexto_listos(), ventana.getTotal_listos());
            ...
            try {
                ...
                if (listos.p.sig != listos.p) {
                    if (hayRecurso()) {//verifica que tenga recursos el proceso en curso para proceder a ejecutarlo
                        ...
                        ventana.getProcesos_en_ejecucion().setText(listos.p.sig.proceso + "");
                        //sleep(2000);
                        ...
                        for (int i = 1; i <= f; i++) {
                            ...
                            sleep(1000);
                            ventana.getTiempo_real().setText((Integer.parseInt(ventana.getTiempo_real().getText()) + 1) + "");
                            cont = cont + 1;
                            if (cont == 90) {
                                //JOptionPane.showMessageDialog(null, "Tiempo Cumplido");
                                ventana.getjButton10().setEnabled(false);
                                ventana.getjButton11().setEnabled(false);
                                stop();
                            }
                            ventana.getCpu_nuevo().setText(listos.p.sig.tiempo_final + "");
                            listos.p.sig.setTiempo_cpu(listos.p.sig.tiempo_cpu + 1);
                            ventana.getTiempo_cpu().setText(listos.p.sig.tiempo_cpu + "");
                            ventana.getProcesos_en_ejecucion().setText(listos.p.sig.proceso + "");
                            listos.imprimirListos(ventana.getTexto_listos(), ventana.getTotal_listos());
                            ...
                            if (listos.num > 1) {//revisa si hay algo en cola de listos y no en ejecución y actualiza su tiempo
                                ...
                                for (int li = 2; li <= listos.num; li++) {
                                    ...
                                    diagramaGantt(auxlis, 2, Integer.parseInt(ventana.getTiempo_real().getText()));
                                    auxlis = auxlis.sig;
                                    listos.imprimirListos(ventana.getTexto_listos(), ventana.getTotal_listos());
                                }
                            }
                            ...
                            if (suspendidos.p.sig != suspendidos.p) {//revisa si hay algo en cola de suspendidos y actualiza su tiempo                                
                                ...
                                for (int k = 1; k <= x; k++) {
                                    ...
                                    diagramaGantt(auxiliar, 4, Integer.parseInt(ventana.getTiempo_real().getText()));
                                    suspendidos.imprimir(ventana.getTexto_suspendidos(), ventana.getTotal_suspendidos());
                            ...
                            if (bloqueados.p.sig != bloqueados.p) {//revisa si hay algo en cola de bloqueados y actualiza su tiempo
                                ...
                                for (int j = 1; j <= bloqueados.num; j++) {
                                    ...
                                    diagramaGantt(auxiliar_b, 3, Integer.parseInt(ventana.getTiempo_real().getText()));
                                    //JOptionPane.showMessageDialog(null, "Proceso " + auxiliar_b.proceso + " bloqueado.");
                                    bloqueados.imprimir(ventana.getTexto_bloqueados(), ventana.getTotal_bloqueados());
                            ...
                            diagramaGantt(listos.p.sig, 1, Integer.parseInt(ventana.getTiempo_real().getText()));
                            ...
                            if (cumpleQuantum(listos.p.sig)) {//verifica que cumpla el quantum y agregue a cola de atendidos                                
                                terminar(listos.p.sig.proceso, Integer.parseInt(ventana.getTiempo_real().getText()));
                                ventana.getTiempo_cpu().setText(listos.p.sig.tiempo_cpu + "");
                                //sleep(2000);
                                ...
                                i = f;
                                atendidos.imprimir();
                                ventana.getTiempo_cpu().setText("");
                                ventana.getProcesos_en_ejecucion().setText("");
                                ventana.getCpu_nuevo().setText("");
    ...
    public void terminar(int num_proceso, int transcurrido) {//actualiza la priemra parte edl diagrama de gantt con el tiempo de los procesos que pasan por cpu
        if (listos.p.sig.tiempo_cpu == listos.p.sig.tiempo_final) {// Si el tiempo de cpu es igual al tiempo final, el proceso ha sido atendido
            ...
            ventana.getTexto_diagrama().setText(diagramaGant1 + "\n");
        }

    }

    public void diagramaGantt(Nodo actual, int estado, int transcurrido) {//actualiza diagrama de gantt desde la ventana gráfica
        String fase;
        ...
        ventana.getjTable1().setValueAt(fase, actual.proceso - 1, transcurrido);
        ventana.getjTable1().setDefaultRenderer(Object.class, new MiRender());
